...
export function SearchPage() {
  ...
  const handleKeyPress = useCallback((e: KeyboardEvent) => {
    if (e.key === 'Backspace') {
      ...
      setSelectedIndex(prev => Math.min(prev + 4, searchResults.length - 1))
      ...
      <div className="flex flex-col h-screen bg-black text-white p-8">
        <div className="mb-8">
          <div className="text-4xl h-16">
            {searchQuery || <span className="text-gray-500">Start typing to search...</span>}
          </div>
        </div>
        ...
        {searchResults.length > 0 ? (
          ...
          <div className="flex-grow flex items-center justify-center">
            {loading ? (
              ...
              <p className="text-2xl text-gray-400">No results found for "{searchQuery}"</p>
            ) : null}
          </div>
        )}
      </div>
