...
import { ArrowLeft, Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Maximize, Minimize, HelpCircle, Loader2 } from 'lucide-react'
...
export function Player({ streamUrl, title, episode, onBack }: DidneyPlusPlayerProps) {
        ...
        <>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black to-transparent">
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <div className="flex items-center space-x-4">
                <span className="text-white text-sm">{title} {episode && `- ${episode}`}</span>
...
export function PlayerPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`/api/catalog/contents/${id}`, {
          method: 'GET',
          headers: {
  ...
  return <Player streamUrl={data?.streamingUrl} title={data?.content?.title} onBack={() => router.back()} />
}